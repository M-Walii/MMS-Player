"""Generate MMS Prime using quaternion-based inflections (H50-T50 distribution)."""
...
_DEFAULT_INPUT_MMS  = r"C:/Users/walee/Documents/Thesis/MMS-Player/MMS-examples/GermanyMapWordH50T50RotationDifference.mms.csv"
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_h50_t50_rotation_difference.py",
...
def parse_arguments() -> Any:
    """Parse CLI arguments."""
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime with quaternion inflections (H50-T50)')
...
def apply_inflections(
    ...
    """Apply quaternion inflections with H50-T50 distribution."""
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            print(f"\n{'='*60}")
            print(f"Line {line_num}: INDEX inflection calculated (H50-T50)")
            print(f"{'='*60}")
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"QUATERNION METHOD - MMS Prime Generation (H50-T50)")
        print(f"{'='*60}")
