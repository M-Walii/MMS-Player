"""Generate MMS Prime using quaternion-based inflections (100% to dominant hand)."""
...
_DEFAULT_INPUT_MMS  = r"C:/Users/walee/Documents/Thesis/MMS-Player/MMS-examples/GermanyMapWordH100RotationDifference.mms.csv"
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_rotation_difference.py",
...
def parse_arguments() -> Any:
    """Parse CLI arguments."""
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime with quaternion inflections')
...
def load_trajectory_data(trajectory_file_path: str) -> Tuple[np.ndarray, np.ndarray]:
    """Load INDEX trajectory and return (starting_point, initial_vector)."""
...
def apply_inflections(
    ...
    """Apply quaternion inflections, 100% to dominant hand rotation."""
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            print(f"\n{'='*60}")
            print(f"Line {line_num}: INDEX inflection calculated")
            print(f"{'='*60}")
            ...
            print(f"  Target vector (normalized): [{(last_target[0]-starting_point[0])/np.linalg.norm(last_target-starting_point):.6f}, "
                  f"{(last_target[1]-starting_point[1])/np.linalg.norm(last_target-starting_point):.6f}, "
                  f"{(last_target[2]-starting_point[2])/np.linalg.norm(last_target-starting_point):.6f}]")
            ...
            print(f"\n  MMS Prime Update:")
            print(f"    domhandrotx = {alpha:.6f}")
            print(f"    domhandroty = {beta:.6f}")
            print(f"    domhandrotz = {gamma:.6f}")
            print(f"{'='*60}\n")
            
            # Apply 100% to dominant hand (all three angles)
            mms_prime.at[line_num, 'domhandrotx'] = alpha
            mms_prime.at[line_num, 'domhandroty'] = beta
            mms_prime.at[line_num, 'domhandrotz'] = gamma
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"QUATERNION METHOD - MMS Prime Generation")
        print(f"{'='*60}")
        ...
        output_file_path = os.path.join(
            args.output_dir,
            os.path.basename(args.input_mms).replace('.mms.csv', '.mms_prime.csv')
        )
