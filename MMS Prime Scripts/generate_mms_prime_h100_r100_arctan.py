"""
Generate MMS Prime from MMS data with inflections - H100-R100 condition.

This script implements test condition #2: H100-R100
- domhandrot[y|z]: 100% of calculated angles
- domhandreloca[y|z]: 100% of calculated angles  
- torsoreloca[y|z]: 0%

Example:
    python generate_mms_prime_h100_r100_arctan.py --input-mms "path/to/mms.csv" --corpus-dir "path/to/corpus" 
...
def parse_arguments() -> Any:
    ...
    """
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections - H100-R100')
...
def apply_inflections(
    ...
    """
    Apply inflection angles to the MMS data to generate MMS Prime - H100-R100 condition.
    Uses INDEX row's trajectory data for all cities, and pointing_dict for target points.
    Angles are applied to both domhandrot and domhandreloca at 100% each.
    """
    mms_prime = mms_df.copy()
    
    print("\nApplying inflections to generate MMS Prime (H100-R100 condition)...")
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if line_num in pointing_dict:
            ...
            target_point = local_h[:3]            # strip w component
            print(f"  Converted world→bone local target: {target_point}")

        if maingloss == 'INDEX' and target_point is not None:
            try:
                print(f"  Using target_point for inflection: {target_point}")
                ...
                # H100-R100 condition: Apply 100% angles to both domhandrot and domhandreloca
                hand_alpha = alpha
                hand_beta = beta
                reloc_alpha = alpha
                reloc_beta = beta

                print(f"  Hand rotation angles (100%): alpha={hand_alpha}, beta={hand_beta}")
                print(f"  Hand relocation angles (100%): alpha={reloc_alpha}, beta={reloc_beta}")
                ...
                # Update hand relocation (100% of total rotation)
                mms_prime.at[line_num, 'domhandrelocay'] = float(reloc_alpha)
                mms_prime.at[line_num, 'domhandrelocaz'] = float(reloc_beta)
                
                print(f"  Updated hand rotation and relocation in INDEX row {line_num}")
                target_point = None
            except Exception as e:
            ...
            if maingloss != 'INDEX':
                ...
                print(f"  Skipping INDEX row - target_point is None")
...
def main() -> None:
    ...
    try:
        ...
        print(f"\nSaved inflected MMS Prime (H100-R100) to: {output_file_path}")
