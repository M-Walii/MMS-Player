"""
Generate MMS Prime from MMS data with inflections (angle_signed 2D).
- alpha1: horizontal angle in XZ (uses (x,z))
- beta1 : vertical   angle in YX (uses (y,x))
Applies 100% to dominant hand: rotx=0, roty=alpha1, rotz=beta1.
"""
...
_DEFAULT_INPUT_MMS  = r"C:/Users/walee/Documents/Thesis/MMS-Player/MMS-examples/ApplyInflectionsH100AngleSigned.mms.csv"
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_angle_signed.py",
...
def parse_arguments() -> Any:
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections (angle_signed 2D)')
...
def compute_inflection_angles_angle_signed(
    ...
    """
    Compute alpha1 (XZ) and beta1 (YX) using 2D signed angles.
    - initial_unit1 = normalize(initial_vector)
    - target_unit1  = normalize(target_point - starting_point)
    - alpha1 = angle_signed( (initial.x,initial.z), (target.x,target.z) )
    - beta1  = angle_signed( (initial.y,initial.x), (target.y,target.x) )
    """
...
def apply_inflections(
    ...
    """
    Uses INDEX trajectory for reference.
    For each line:
      - If line index exists in pointing_dict → remember target (world→bone)
      - When current row maingloss == 'INDEX' and we have a remembered target:
          compute alpha1/beta1 and write to domhandroty/rotz (rotx=0)
    """
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            # Apply 100% to dominant hand
            mms_prime.at[line_num, 'domhandrotx'] = 0.0
            mms_prime.at[line_num, 'domhandroty'] = alpha1
            mms_prime.at[line_num, 'domhandrotz'] = beta1
