"""
Generate MMS Prime from MMS data with inflections - H50-T50 condition with multi-order solver.

This script implements test condition #3: H50-T50 with adaptive angle solver
- domhandrot[y|z]: 50% of calculated angles
- domhandreloca[y|z]: 0%
- torsoreloca[y|z]: 50% of calculated angles

Uses dual-order angle solver for better accuracy across all directions.

Example:
    python generate_mms_prime_h50_t50_multi_order.py --input-mms "path/to/mms.csv" --corpus-dir "path/to/corpus" 
...
def parse_arguments() -> Any:
    ...
    """
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections - H50-T50 Multi-Order')
...
def apply_inflections(
    ...
    """
    Apply inflection angles to the MMS data to generate MMS Prime - H50-T50 condition.
    Uses INDEX row's trajectory data for all cities, and pointing_dict for target points.
    Angles are split 50/50 between domhandrot and torsoreloca.
    """
    mms_prime = mms_df.copy()
    
    print("\nApplying inflections to generate MMS Prime (H50-T50 Multi-Order)...")
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            try:
                ...
                # H50-T50 condition: Split angles 50/50 between domhandrot and torsoreloca
                hand_alpha = alpha * 0.5
                hand_beta = beta * 0.5
                torso_alpha = alpha * 0.5
                torso_beta = beta * 0.5

                print(f"  Hand rotation angles (50%): alpha={hand_alpha}, beta={hand_beta}")
                print(f"  Torso relocation angles (50%): alpha={torso_alpha}, beta={torso_beta}")
                ...
                print(f"  Updated hand rotation and torso relocation in INDEX row {line_num}")
                last_target = None
            except Exception as e:
...
def main() -> None:
    ...
    try:
        ...
        print(f"\nSaved inflected MMS Prime (H50-T50 Multi-Order) to: {output_file_path}")
