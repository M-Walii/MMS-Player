"""Generate MMS Prime using quaternion inflections for DURCHFAHREN2 with torso compensation (H75-R75-T25)."""
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_durchfahren2_h75_r75_t25_rotation_difference_torso_comp.py",
...
def parse_arguments() -> Any:
    """Parse CLI arguments."""
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime with quaternion inflections for DURCHFAHREN2 (H75-R75-T25, torso compensated)')
...
def apply_inflections(
    ...
    """Apply quaternion inflections for DURCHFAHREN2 with torso compensation (H75-R75-T25)."""
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'DURCHFAHREN2':
            ...
            # Compute full inflection angles
            ...
            print(f"\n{'='*60}")
            print(f"Line {line_num}: DURCHFAHREN2 inflection calculated (H75-R75-T25, torso-comp)")
            print(f"{'='*60}")
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"DURCHFAHREN2 QUATERNION METHOD - MMS Prime Generation (H75-R75-T25, torso-comp)")
        print(f"{'='*60}")
