"""
Generate MMS Prime from MMS data with inflections (Quaternion Method - H100-R100).
...
- Distribution: 100% to domhandrot, 100% to domhandreloc
"""
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_h100_r100_rotation_difference.py",
...
def parse_arguments() -> Any:
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections (Quaternion Method - H100-R100)')
...
def apply_inflections(
    ...
    """
    Apply inflections using quaternion method with H100-R100 distribution.
    
    Distribution:
    - 100% to domhandrot (alpha→rotx, beta→roty, gamma→rotz)
    - 100% to domhandreloc (alpha→relocax, beta→relocay, gamma→relocaz)
    
    Process:
    - For each line, if line index exists in pointing_dict → remember target (world→bone)
    - When current row maingloss == 'INDEX' and we have a remembered target:
        compute (α, β, γ) using quaternion method and distribute 100/100
    """
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            print(f"\n{'='*60}")
            print(f"Line {line_num}: INDEX inflection calculated (H100-R100)")
            print(f"{'='*60}")
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"QUATERNION METHOD - MMS Prime Generation (H100-R100)")
        print(f"{'='*60}")
