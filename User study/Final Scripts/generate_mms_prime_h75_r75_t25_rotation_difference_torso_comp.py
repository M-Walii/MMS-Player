"""
Generate MMS Prime from MMS data with inflections (Quaternion Method - H75-R75-T25).
- Uses professor's quaternion rotation_difference method
- Calculates Q = initial_vector.rotation_difference(target_vector)
- Converts Q.to_euler("ZXY") → (α, β, γ)
- Torso compensation per professor's method
"""
...
_DEFAULT_INPUT_MMS  = r"C:/Users/walee/Documents/Thesis/MMS-Player/MMS-examples/0003_mms.csv"
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_h75_r75_t25_rotation_difference_torso_comp.py",
...
# Torso base in Bone_Spine2 space (origin of that bone)
...
def parse_arguments() -> Any:
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections (Quaternion Method - H75-R75-T25, torso compensated)')
...
def compute_inflection_angles_quaternion_method(
    ...
    """
    Compute rotation angles using professor's quaternion method.
    
    Method:
    1. Calculate quaternion Q = initial_vector.rotation_difference(target_vector)
    2. Convert Q.to_euler("ZXY") → (α, β, γ)
    ...
    Returns:
        Tuple[float, float, float]: (alpha, beta, gamma) angles in radians
    """
    # Normalize vectors
...
def compute_torso_compensated_target(
    ...
    """
    Compute compensated target point T' after applying torso rotation.
    """
    # Compose torso rotation matrix in ZXY order
...
def apply_inflections(
    ...
    """
    Apply inflections using quaternion method with torso compensation.
    
    Steps:
    - Compute (α, β, γ) for the full target
    - Torso: αT = α*0.25, βT = β*0.25, γT = γ*0.25
    - Compute T' with inverse torso rotation around torso base
    - Compute hand angles (αH, βH, γH) using T'
    - Apply 75% of hand angles to domhandrot+domhandreloc
    - Apply torso to torsoreloc
    """
    ...
    for line_num, row in mms_df.iterrows():
        # 1) Capture target if present at this line index (world → bone)
        if line_num in pointing_dict:
            ...
            last_target = local_h[:3]                                   # strip w

        # 2) If this is an INDEX row and we have a target, compute/apply
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            # Torso split (25%)
            torso_alpha = alpha * TORSO_FRACTION
            torso_beta = beta * TORSO_FRACTION
            torso_gamma = gamma * TORSO_FRACTION

            # Compensated target and hand angles
            ...
            # Apply 75% to hand angles
            hand_alpha *= HAND_FRACTION
            hand_beta *= HAND_FRACTION
            hand_gamma *= HAND_FRACTION

            # Print calculated values to console
            print(f"\n{'='*60}")
            print(f"Line {line_num}: INDEX inflection calculated (H75-R75-T25, torso-comp)")
            print(f"{'='*60}")
            print(f"  Target point (bone-space): [{last_target[0]:.6f}, {last_target[1]:.6f}, {last_target[2]:.6f}]")
            print(f"  Target' (bone-space):      [{target_prime[0]:.6f}, {target_prime[1]:.6f}, {target_prime[2]:.6f}]")
            print(f"\n  FULL QUATERNION RESULTS:")
            ...
            print(f"\n  TORSO (25%):")
            print(f"    torsorelocax = {torso_alpha:.6f}")
            print(f"    torsorelocay = {torso_beta:.6f}")
            print(f"    torsorelocaz = {torso_gamma:.6f}")
            print(f"\n  HAND (75%, torso-compensated):")
            print(f"    domhandrotx = {hand_alpha:.6f}")
            print(f"    domhandroty = {hand_beta:.6f}")
            print(f"    domhandrotz = {hand_gamma:.6f}")
            print(f"    domhandrelocax = {hand_alpha:.6f}")
            print(f"    domhandrelocay = {hand_beta:.6f}")
            print(f"    domhandrelocaz = {hand_gamma:.6f}")
            print(f"{'='*60}\n")
            ...
            last_target = None  # reset for next target
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"QUATERNION METHOD - MMS Prime Generation (H75-R75-T25, torso-comp)")
        print(f"{'='*60}")
        ...
        output_file_path = os.path.join(
            args.output_dir,
            os.path.basename(args.input_mms).replace('_mms.csv', '_H75R75T25_mms_prime.csv')
        )
