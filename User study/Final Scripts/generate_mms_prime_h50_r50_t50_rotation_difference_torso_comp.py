"""
Generate MMS Prime from MMS data with inflections (Quaternion Method - H50-R50-T50).
...
if _RUNNING_IN_BLENDER and not _BLENDER_BG:
    if len(sys.argv) <= 1 or sys.argv[0].endswith("blender"):
        sys.argv = [
            "generate_mms_prime_h50_r50_t50_rotation_difference_torso_comp.py",
...
def parse_arguments() -> Any:
    import argparse
    parser = argparse.ArgumentParser(description='Generate MMS Prime from MMS data with inflections (Quaternion Method - H50-R50-T50, torso compensated)')
...
def apply_inflections(
    ...
    - Torso: αT = α*0.5, βT = β*0.5, γT = γ*0.5
    - Compute T' with inverse torso rotation around torso base
    - Compute hand angles (αH, βH, γH) using T'
    - Apply torso to torsoreloc and hand angles to domhandrot+domhandreloc
    """
    ...
    for line_num, row in mms_df.iterrows():
        ...
        if maingloss == 'INDEX' and last_target is not None:
            ...
            print(f"\n{'='*60}")
            print(f"Line {line_num}: INDEX inflection calculated (H50-R50-T50, torso-comp)")
            print(f"{'='*60}")
...
def main() -> None:
    try:
        ...
        print(f"\n{'='*60}")
        print(f"QUATERNION METHOD - MMS Prime Generation (H50-R50-T50, torso-comp)")
        print(f"{'='*60}")
